...
read.credentials <- function(f){
    tmp <- read.table(f, stringsAsFactors=FALSE, header=FALSE )
    v <- tmp[,2]
    names(v) <- tmp[,1]
    as.list(v)
}

## modified from my workstation functions to take a named list of connection details
## provided by some means.
ensembl.connect <- function(cred, db.name){
...
get.transcript.exons <- function(cred, db.name, tr){
    query <- paste("select d.name, c.seq_region_start as 'ex_start', c.seq_region_end as 'ex_end', a.seq_region_strand as 'strand', b.rank ",
                   "from transcript a ",
                   "inner join exon_transcript b on a.transcript_id=b.transcript_id ",
                   "and a.stable_id = '", tr, "' ",
                   "inner join exon c on b.exon_id=c.exon_id ",
                   "inner join seq_region d on d.seq_region_id=c.seq_region_id ",
                   "order by b.rank;", sep="")
...
get.transcript.description <- function(cred, db.name, tr){
    query <- paste("select a.stable_id as 'tr.id', b.stable_id as 'gene.id', c.description, c.display_label as 'gene' ",
                   "from transcript a ",
                   "inner join gene b on a.gene_id=b.gene_id and a.stable_id='", tr, "' ",
                   "inner join xref c on b.display_xref_id=c.xref_id;",
                   sep="")
    db <- ensembl.connect(cred, db.name)
    des <- dbGetQuery(db, query)
    dbDisconnect(db)
    des
}

get.region.transcripts <- function(cred, db.name, chr, beg, end){
    query <- paste("select a.stable_id, a.biotype, b.name, a.seq_region_start, a.seq_region_end, ",
                   "c.stable_id, d.rank, e.seq_region_start as 'ex_start', e.seq_region_end as 'ex_end' ",
                   "from gene a ",
                   "inner join seq_region b on a.seq_region_id=b.seq_region_id ",
                   "and b.name = '", chr, "' ",
                   "and a.seq_region_end >=",  beg,  " and a.seq_region_start <= ", end, " ",
                   "inner join transcript c on a.gene_id=c.gene_id ",
                   "inner join exon_transcript d on c.transcript_id=d.transcript_id ",
                   "inner join exon e on d.exon_id=e.exon_id ",
                   "order by a.gene_id, c.transcript_id, d.rank;",
                   sep="")
    db <- ensembl.connect(cred, db.name)
    exons <- dbGetQuery(db, query)
    dbDisconnect(db)
...
get.ens.exon.info <- function(cred, db.name){
...
get.genome.stats <- function(cred, db.name){
...
get.ens.canonical.translations <- function(cred, db.name){
...
get.ensembl.taxonomy <- function(cred, db.name='ensembl_compara_98', root.name='Ensembl'){
...
get.family.members <- function(cred, sp.name, db, db.is.name=FALSE){
    family.query.fmt <-
...
get.all.ref.genes <- function(cred, db.name, max.coord.rank=1){
...
## plots data returned by the mixtools package. Adds on some
## extra functionality and less general plot function
plotMix <- function(mod, legpos='topright', obs.col='black', mod.col='black',
                    obs.lwd=4, mod.lwd=4, obs.lty=3, comp.lwd=1, ...){
    if(class(mod) != 'mixEM' || mod$ft != 'normalmixEM')
        stop("This only works with normal mix models")
    x <- sort(mod$x)
    mod.y <- matrix(nrow=length(x), ncol=length(mod$lambda))
    for(i in 1:ncol(mod.y))
        mod.y[,i] <- dnorm( x, mean=mod$mu[i], sd=mod$sigma[i] ) * mod$lambda[i]
    obs.y <- density(x)
    plot(obs.y$x, obs.y$y, ylim=c(0, max(c(obs.y$y, rowSums(mod.y)))), col=obs.col, lty=obs.lty,
         lwd=obs.lwd, type='l', ylab='density', ...)
    lines(x, rowSums(mod.y), col=mod.col, lwd=mod.lwd)
    for(i in 1:ncol(mod.y))
        lines(x, mod.y[,i], col=i+1, lwd=comp.lwd)
    if(!is.null(legpos))
        legend(legpos, legend=c('observed', 'model'), lty=c(obs.lty,1),
               lwd=c(obs.lwd,mod.lwd), col=c(obs.col, mod.col)) 
}
